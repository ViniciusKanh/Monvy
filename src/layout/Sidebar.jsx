  ...
  return (
    <div className="h-full flex flex-col bg-gradient-to-b from-[#080d1f] to-[#0d1433] text-slate-300 w-72">
      <div className="px-5 py-5">
        <Logo size="md" />
        <p className="text-[10px] tracking-widest text-slate-500 mt-1 ml-11 -mt-1">{t('ui.tagline')}</p>
      </div>
      ...
      <div className="mx-4 mb-4 p-3 rounded-xl bg-white/5 border border-white/10 flex items-center gap-3">
        ...
        <div className="min-w-0">
          <p className="text-sm font-semibold text-white truncate">{user?.full_name || t('ui.user')}</p>
          <p className="text-[11px] text-emerald-400 flex items-center gap-1">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
            {user?.role === 'admin' ? t('ui.role_admin') : t('ui.role_user')}
          </p>
        </div>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto px-3 pb-4 space-y-4">
        {NAV_GROUPS.map((group) => {
          ...
          return (
            <div key={group.label}>
              <p className="px-3 text-[10px] font-bold tracking-widest text-slate-500 mb-1.5">{t('group.' + group.label, group.label).toUpperCase()}</p>
              <div className="space-y-0.5">
                {items.map((it) => (
                  <NavLink key={it.key} to={it.path} end={it.path === '/'} onClick={onNavigate}
                    ...
                    <span className="truncate">{t('nav.' + it.key, it.label)}</span>
                  </NavLink>
                ))}
              </div>
            </div>
      ...
      <button onClick={logout}
        ...
        <LogOut className="w-[18px] h-[18px]" /> {t('ui.logout')}
      </button>
    </div>
  );
}
