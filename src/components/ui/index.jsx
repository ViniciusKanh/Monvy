...
export function Modal({ open, onClose, title, children, footer, maxWidth = 'max-w-lg' }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className={cn('relative w-full card p-0 rounded-t-2xl sm:rounded-2xl overflow-hidden animate-[fadeIn_.15s_ease]', maxWidth)}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-[hsl(var(--border))]">
          <h3 className="font-display font-bold text-lg">{title}</h3>
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-black/5 dark:hover:bg-white/10"><X className="w-5 h-5" /></button>
        </div>
        <div className="px-5 py-4 max-h-[70vh] overflow-y-auto">{children}</div>
        {footer && <div className="px-5 py-4 border-t border-[hsl(var(--border))] flex justify-end gap-2">{footer}</div>}
      </div>
    </div>
  );
}
