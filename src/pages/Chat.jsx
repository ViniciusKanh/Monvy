          ...
          {messages.map((m, i) => m.role === 'council' ? (
            ...
            <div key={i} className={`flex gap-3 ${m.role === 'user' ? 'flex-row-reverse' : ''}`}>
              ...
              <div className="max-w-[80%]">
                ...
                <div className={`rounded-2xl px-4 py-2.5 text-sm ${m.role === 'user' ? 'bg-emerald-500 text-white whitespace-pre-wrap' : 'bg-black/5 dark:bg-white/5'}`}>{m.role === 'assistant' ? <Markdown text={m.text} /> : m.text}</div>
