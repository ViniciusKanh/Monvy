...
import { Users as UsersIcon, ShieldCheck, Trash2, SlidersHorizontal } from 'lucide-react';
...
export default function Users() {
  ...
  const openEdit = (u) => { setEditing(u); setScreens(u.allowed_screens || []); setRole(u.role); };
  ...
  return (
    <div>
      ...
      <Modal open={!!editing} onClose={() => setEditing(null)} title={`Acessos: ${editing?.full_name || editing?.email || ''}`} maxWidth="max-w-2xl"
        footer={<><Button variant="outline" onClick={() => setEditing(null)}>Cancelar</Button><Button onClick={() => save.mutate({ id: editing.id, data: { role, allowed_screens: role === 'admin' ? undefined : screens } })} disabled={save.isPending}>{save.isPending ? <Spinner className="w-4 h-4" /> : 'Salvar'}</Button></>}>
