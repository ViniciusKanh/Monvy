...
export default function Fire() {
  ...
  return (
    <div className="space-y-5 animate-fadeIn">
      <PageHeader title={<span className="flex items-center gap-2"><Flame className="w-6 h-6 text-orange-500" /> Independencia Financeira (FIRE)</span>}
      ...
      <div className="flex items-start gap-2 text-xs p-3 rounded-xl bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-300 border border-amber-500/20">
        ...
        <span>Ideia da regra dos 4% (multiplo 25x): quando seu patrimonio investido chega a ~25x seu gasto anual, os rendimentos tendem a cobrir suas despesas para sempre. Isto e uma <b>estimativa</b> com base nos numeros que voce informar — nao e garantia nem recomendacao de investimento.</span>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        <Card className="hover-lift">
          ...
          <div className="space-y-3">
            ...
            <Field label="Gasto mensal (R$)" hint="quanto voce gasta por mes"><Input type="number" value={gastoMensal} onChange={(e) => setGastoMensal(e.target.value)} /></Field>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="relative overflow-hidden rounded-3xl p-6 text-white shadow-soft" style={{ background: 'linear-gradient(135deg,#7c2d12 0%,#b45309 50%,#f59e0b 100%)' }}>
            ...
            <div className="relative">
              ...
              <p className="text-amber-50 text-sm mt-1">{n(multiplo) || 25}x seu gasto anual ({formatCurrency(n(gastoMensal) * 12)}). Rende ~{formatCurrency(plan.monthlyIncomeAtFI)}/mes.</p>
              <div className="mt-4 inline-flex items-center gap-2 bg-white/15 rounded-xl px-4 py-2">
                <Flame className="w-5 h-5" />
                {plan.reached ? <span className="font-bold text-lg">Voce atinge em {plan.years} ano(s){plan.rem ? ` e ${plan.rem} mes(es)` : ''}</span>
                  : <span className="font-semibold">Com estes numeros, o alvo nao e atingido em 80 anos — aumente o aporte ou o retorno.</span>}
              </div>
            </div>
          </div>

          <Card className="hover-lift">
            <h3 className="font-semibold mb-2">Projecao do patrimonio</h3>
