...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!token || !password) return sendJson(res, 400, { error: 'Dados incompletos' });
    ...
    const u = r.rows[0];
    if (!u) return sendJson(res, 400, { error: 'Link invalido ou ja utilizado' });
    if (u.reset_expires && new Date(u.reset_expires) < new Date()) return sendJson(res, 400, { error: 'Link expirado. Solicite um novo.' });
    ...
    sendMail({ to: u.email, subject: 'Senha redefinida — Monvy', html: tpl('Sua senha foi redefinida', `Sua senha foi alterada em ${new Date().toLocaleString('pt-BR')}. Se nao foi voce, entre em contato imediatamente.`) }).catch(() => {});
