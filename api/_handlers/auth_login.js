...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (user.totp_enabled === 1) {
      if (!code) return sendJson(res, 401, { error: '2FA_REQUIRED', message: 'Informe o codigo do app autenticador.' });
      if (!verifyTotp(code, user.totp_secret)) return sendJson(res, 401, { error: '2FA_INVALID', message: 'Codigo de verificacao invalido.' });
    }
...
function publicUser(u) {
  ...
  return {
    ...
    totp_enabled: u.totp_enabled === 1, require_2fa: u.require_2fa === 1,
  };
}
